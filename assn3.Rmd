...
cluster_frequency <- c(unlist(lapply(PAM_cluster_10000genes$clusinfo[,1], function(x) x / 10000)), 
                       unlist(lapply(PAM_cluster_1000genes$clusinfo[,1], function(x) x / 1000)), 
                       unlist(lapply(PAM_cluster_100genes$clusinfo[,1], function(x) x / 100)), 
                       unlist(lapply(PAM_cluster_10genes$clusinfo[,1], function(x) x / 10))
                       )
...
# Cannot figure out how to change the colors.
ggplot(as.data.frame(alluvial_df),
       aes(y = ClusterFrequencyPercentage, axis1 = GeneCount, axis2 = ClusterNumber)) +
  geom_alluvium(width = 1/12) +
  geom_stratum(width = 1/12, fill = "black", color = "grey") +
  geom_label(stat = "stratum", aes(label = after_stat(stratum))) +
  scale_x_discrete(limits = c("Gene Count", "Cluster Number"), expand = c(.05, .05)) +
  scale_fill_brewer(type = "qual", palette = "Set1") +
  ggtitle("Cluster Frequency Percentage")
