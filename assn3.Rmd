...
required_packages <- c("DESeq2", "readr", "magrittr", "ggplot2", "dplyr", "ConsensusClusterPlus", "cluster", "factoextra", "pheatmap", "ggalluvial", "mclust")
...
hierarchical_result_10 <- hclust(distance_matrix, method = "complete")
par(cex=0.8)
plot(hierarchical_result_10, hang = -1, main = "Hierarchical Clustering Dendrogram (10 Genes)")
dev.off()
par(cex=0.8)
plot(hierarchical_result_10, hang = -1, main = "Hierarchical Clustering Dendrogram (10 Genes)")
...
hierarchical_result_100 <- hclust(distance_matrix, method = "complete")
par(cex=0.8)
plot(hierarchical_result_100, hang = -1, main = "Hierarchical Clustering Dendrogram (100 Genes)")
dev.off()
par(cex=0.8)
plot(hierarchical_result_100, hang = -1, main = "Hierarchical Clustering Dendrogram (100 Genes)")
...
hierarchical_result_1000 <- hclust(distance_matrix, method = "complete")
par(cex=0.8)
plot(hierarchical_result_1000, hang = -1, main = "Hierarchical Clustering Dendrogram (1000 Genes)")
dev.off()
par(cex=0.8)
plot(hierarchical_result_1000, hang = -1, main = "Hierarchical Clustering Dendrogram (1000 Genes)")
...
hierarchical_result_10000 <- hclust(distance_matrix, method = "complete")
par(cex=0.8)
plot(hierarchical_result_10000, hang = -1, main = "Hierarchical Clustering Dendrogram (10000 Genes)")
dev.off()
par(cex=0.8)
plot(hierarchical_result_10000, hang = -1, main = "Hierarchical Clustering Dendrogram (10000 Genes)")
...
```{r}
gene_count_hclust <- rep(c("10000", "1000", "100", "10"), each = 5)
cluster_num_hclust <- rep(1:5, times = 4)

# Calculate the cluster frequencies
clusters_10 <- cutree(hierarchical_result_10, k = 5)
clusters_100 <- cutree(hierarchical_result_100, k = 5)
clusters_1000 <- cutree(hierarchical_result_1000, k = 5)
clusters_10000 <- cutree(hierarchical_result_10000, k = 5)
cluster_frequency_hclust <- as.vector(table(c(clusters_10000, clusters_1000, clusters_100, clusters_10)) / 60)

# Create a data frame with the information
data <- data.frame(
  GeneCount = gene_count_hclust,
  ClusterNumber = factor(as.character(cluster_num_hclust)),  # Convert to factor
  ClusterFrequency = cluster_frequency_hclust
)
...
alluvial_plot <- ggplot(data, aes(y = ClusterFrequency, axis1 = GeneCount, axis2 = ClusterNumber, fill = ClusterNumber)) +
  geom_alluvium(width = 1/12) +
  geom_stratum(width = 1/12, color = "grey") +
  geom_text(stat = "stratum", aes(label = after_stat(stratum))) +
  scale_x_discrete(limits = c("Gene Count", "Cluster Number"), expand = c(.05, .05)) +
  scale_fill_brewer(type = "qual", palette = "Set1") +
  ggtitle("Hierarchical Clustering Frequency Distribution for k = 5")

ggsave("Plots/assignment3/Hclust_Alluvial_Plot.png", plot = last_plot(), width = 8, height = 6, units = "in", dpi = 300)
```
...
annotation_data <- data.frame(
  KMeansCluster = metadata$KMeans,
  PAMCluster = metadata$PAMCluster,
  HierarchicalCluster = metadata$HClust,
  SampleGroups = metadata$Group
)

png("Plots/assignment3/heatmap_clust_annotations_2.png", width = 800, height = 800)
...
# 5000 genes
metadata$KMeans2_5000 <- km.res2.5000$cluster
metadata$HClust3_5000 <- cutree(hierarchical_result, k = 3)
metadata$PAMCluster3_5000 <- PAM_cluster_3$clustering

metadata$KMeans3_5000 <- km.res3.5000$cluster
metadata$HClust5_5000 <- cutree(hierarchical_result, k = 5)
metadata$PAMCluster5_5000 <- PAM_cluster_5$clustering

metadata$KMeans4_5000 <- km.res4.5000$cluster
metadata$HClust10_5000 <- cutree(hierarchical_result, k = 10)
metadata$PAMCluster10_5000 <- PAM_cluster_10$clustering

# 10 genes
metadata$KMeans3_10 <- km.res3.10$cluster
metadata$HClust5_10 <- cutree(hierarchical_result_10, k = 5)
metadata$PAMCluster5_10 <- PAM_cluster_10genes$clustering

# 100 genes
metadata$KMeans3_100 <- km.res3.100$cluster
metadata$HClust5_100 <- cutree(hierarchical_result_100, k = 5)
metadata$PAMCluster5_100 <- PAM_cluster_100genes$clustering

# 1000 genes
metadata$KMeans3_1000 <- km.res3.1000$cluster
metadata$HClust5_1000 <- cutree(hierarchical_result_1000, k = 5)
metadata$PAMCluster5_1000 <- PAM_cluster_1000genes$clustering

# 10000 genes
metadata$KMeans3_10000 <- km.res3.10000$cluster
metadata$HClust5_10000 <- cutree(hierarchical_result_10000, k = 5)
metadata$PAMCluster5_10000 <- PAM_cluster_10000genes$clustering
```

```{r}

# Create a function to perform a chi-squared test
perform_chi_squared_test <- function(data, cluster_column, group_column) {
  contingency_table <- table(data[[cluster_column]], data[[group_column]])
...
clustering_results <- list(
  KMeans2_5000 = perform_chi_squared_test(metadata, "KMeans2_5000", "Group"),
  KMeans3_5000 = perform_chi_squared_test(metadata, "KMeans3_5000", "Group"),
  KMeans4_5000 = perform_chi_squared_test(metadata, "KMeans4_5000", "Group"),
  KMeans3_10 = perform_chi_squared_test(metadata, "KMeans3_10", "Group"),
  KMeans3_100 = perform_chi_squared_test(metadata, "KMeans3_100", "Group"),
  KMeans3_1000 = perform_chi_squared_test(metadata, "KMeans3_1000", "Group"),
  KMeans3_10000 = perform_chi_squared_test(metadata, "KMeans3_10000", "Group"),
  HClust3_5000 = perform_chi_squared_test(metadata, "HClust3_5000", "Group"),
  HClust5_5000 = perform_chi_squared_test(metadata, "HClust5_5000", "Group"),
  HClust10_5000 = perform_chi_squared_test(metadata, "HClust10_5000", "Group"),
  HClust5_10 = perform_chi_squared_test(metadata, "HClust5_10", "Group"),
  HClust5_100 = perform_chi_squared_test(metadata, "HClust5_100", "Group"),
  HClust5_1000 = perform_chi_squared_test(metadata, "HClust5_1000", "Group"),
  HClust5_10000 = perform_chi_squared_test(metadata, "HClust5_10000", "Group"),
  PAMCluster3_5000 = perform_chi_squared_test(metadata, "PAMCluster3_5000", "Group"),
  PAMCluster5_5000 = perform_chi_squared_test(metadata, "PAMCluster5_5000", "Group"),
  PAMCluster10_5000 = perform_chi_squared_test(metadata, "PAMCluster10_5000", "Group"),
  PAMCluster5_10 = perform_chi_squared_test(metadata, "PAMCluster5_10", "Group"),
  PAMCluster5_100 = perform_chi_squared_test(metadata, "PAMCluster5_100", "Group"),
  PAMCluster5_1000 = perform_chi_squared_test(metadata, "PAMCluster5_1000", "Group"),
  PAMCluster5_10000 = perform_chi_squared_test(metadata, "PAMCluster5_10000", "Group")
)

# Extract p-values from the chi-squared test results
p_values <- sapply(clustering_results, function(result) result$p.value)
...
test_results <- data.frame(
  Clustering_Method = names(clustering_results),
  P_Value = p_values,
